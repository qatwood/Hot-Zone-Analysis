...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    // All Rectangles with their count, sorted by "rectangle" string
    val joinResultWithCount = spark.sql("select rectangle,count(rectangle) as recCount from joinResult group by rectangle order by rectangle").coalesce(1)
    
    return joinResultWithCount 
  }
